...
def create_logger(args):
    ...
    """
    Create and configure a logger for training or evaluation.

    Parameters:
    - args: An argparse.Namespace object containing script arguments.

    Returns:
    - logger: A configured logger for logging training or evaluation information.
    """
    ...
    hms = '-'.join(list(map(str, [datenow.hour, datenow.minute, datenow.second])))  # Replaced ':' with '-'

    # Create a unique log name based on the date and time
    ...
    # Extract checkpoint name from the evaluation checkpoint path
    ckpt_name = Path(args.eval_ckpt).stem.split('checkpoint_model_epoch_')[-1]

    # Determine log type (train or eval) based on the 'evaluate' flag
    ...
    logging.basicConfig(
        ...
        datefmt='%a, %d-%b-%Y %H:%M:%S',  # Corrected the format here
        filename=filename,
        filemode='w'
    )
...
def define_args():
    ...
    parser.add_argument('--nworkers', type=int,
                        default=0, help='num of threads')
    parser.add_argument('--test_mode', action='store_true',
                        help='prevents loading latest saved model')
    parser.add_argument('--start_epoch', type=int, default=0,
                        help='prevents loading latest saved model')
    parser.add_argument('--evaluate', action='store_true',
                        default=False, help='only perform evaluation')
    parser.add_argument('--vis', action='store_true')
    parser.add_argument('--resume', type=str, default='',
                        help='resume latest saved run')
...
def prune_3d_lane_by_range(lane_3d, x_min, x_max):
    ...
    lane_3d = lane_3d[np.logical_and(
        lane_3d[:, 1] > 0, lane_3d[:, 1] < 200), ...]
...
def resample_laneline_in_y(input_lane, y_steps, out_vis=False):
    ...
    if input_lane.shape[1] < 3:
        input_lane = np.concatenate([input_lane, np.zeros(
            [input_lane.shape[0], 1], dtype=np.float32)], axis=1)

    f_x = interp1d(input_lane[:, 1], input_lane[:, 0],
                   fill_value="extrapolate")
    f_z = interp1d(input_lane[:, 1], input_lane[:, 2],
                   fill_value="extrapolate")
...
def homograpthy_g2im(cam_pitch, cam_height, K):
    ...
    R_g2c = np.array([[1, 0, 0],
                      [0, np.cos(np.pi / 2 + cam_pitch), -
                       np.sin(np.pi / 2 + cam_pitch)],
                      [0, np.sin(np.pi / 2 + cam_pitch), np.cos(np.pi / 2 + cam_pitch)]])
    H_g2im = np.matmul(K, np.concatenate(
        [R_g2c[:, 0:2], [[0], [cam_height], [0]]], 1))
    return H_g2im
...
def define_scheduler(optimizer, args, dataset_size=None):
    if args.lr_policy == 'lambda':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + 1 - args.niter) / \
                float(args.niter_decay + 1)
            return lr_l
    ...
    #                                                patience=args.lr_decay_iters)
        ...
        cosine_warmup = partial(
            cosine_schedule_with_warmup, args=args, dataset_size=dataset_size)
